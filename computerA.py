...
class ComputerA(Tools):
    ...
    def on_message(self, client: mqtt.Client, userdata, msg: mqtt.MQTTMessage):
        ...
        if msg.topic == self.topic:
            try:
                # Parse and store the public key (B) from Computer B
                self.B = int(msg.payload.decode('utf-8')) # Set B from the received public key
                if self.send_mode:
    ...
    def encrypt_message(self, message: str) -> str:
        """
        Encrypt the given message using the public key (B) received from Computer B.
...
computer_a = ComputerA(broker, port, topic, topic_encryption, 2137, 5, "deneme", False, True)
computer_a.run()